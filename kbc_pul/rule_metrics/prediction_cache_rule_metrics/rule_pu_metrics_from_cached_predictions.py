...
from kbc_pul.rule_metrics.prediction_cache_rule_metrics.rule_pu_confidence_pca_from_cached_predictions import \
    calculate_pu_propensity_confidence_pca_from_df_cache
from kbc_pul.rule_metrics.prediction_cache_rule_metrics.rule_pu_confidence_unbiased_from_cached_predictions import \
    calculate_pu_propensity_confidence_unbiased_from_df_cache, get_inverse_propensity_weighted_count_of_predictions, \
    get_inverse_propensity_score_per_prediction


def calculate_rule_pu_metrics_from_df_cached_predictions(
    ...
    if len(df_cached_predictions) != 0:
        ...
        pu_conf_unbiased: float = calculate_pu_propensity_confidence_unbiased_from_df_cache(
        ...
        pu_conf_pca_subject_to_object: float = calculate_pu_propensity_confidence_pca_from_df_cache(
            df_cached_predictions=df_cached_predictions,
            rule_head=rule_head,
            pylo_context=pylo_context,
        ...
        pu_conf_pca_object_to_subject = calculate_pu_propensity_confidence_pca_from_df_cache(
            df_cached_predictions=df_cached_predictions,
            rule_head=rule_head,
            pylo_context=pylo_context,
