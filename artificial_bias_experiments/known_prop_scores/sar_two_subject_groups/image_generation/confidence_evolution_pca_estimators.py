...
def known_prop_scores_sar_two_subject_groups_plot_conf_evolution_true_conf_star_vs_pca_estimators_per_rule(
    ...
    dir_true_conf_vs_pca_confidence_estimators: str = os.path.join(
        image_dir,
        "confidence_evolution_true_conf_star_vs_pca_estimators"
    )
    ...
    for rule_name in df_rule_wrappers["Rule"].unique():
        df_pca_conf_comp_melted_single_rule = df_pca_conf_comp[
            df_pca_conf_comp["Rule"] == rule_name
            ]
        df_pca_conf_comp_melted = df_pca_conf_comp_melted_single_rule.melt(
        ...
        df_diff_to_true_conf_melted: pd.DataFrame = get_df_diffs_between_true_conf_and_confidence_estimators_melted(
            df_rule_wrappers=df_pca_conf_comp_melted_single_rule,
        ...
        # plt.legend(bbox_to_anchor=(-0.05, 1.0), loc='upper right')
